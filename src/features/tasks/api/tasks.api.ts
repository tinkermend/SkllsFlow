...
export async function deleteTask(taskUuid: TaskUuid): Promise<void> {
  await apiClient.delete(API_ENDPOINTS.tasks.delete(taskUuid));
}

export async function runTask(taskUuid: TaskUuid): Promise<TaskRun> {
...
export async function pauseTask(taskUuid: TaskUuid): Promise<Task> {
...
export async function resumeTask(taskUuid: TaskUuid): Promise<Task> {
  const response = await apiClient.post<Task>(
...
export async function getTaskRuns(taskUuid: TaskUuid): Promise<TaskRun[]> {
...
export async function getTaskRun(runUuid: TaskRunUuid): Promise<TaskRun> {
  const response = await apiClient.get<TaskRun>(
