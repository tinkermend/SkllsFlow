...
export function SkillCard({
  ...
  const statusConfig = {
    [SkillStatus.ACTIVE]: {
      ...
      className: 'bg-green-500/10 text-green-600 hover:bg-green-500/20 shadow-none border-0',
    },
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="space-y-1 pt-0.5 min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span className="shrink-0">{new Date(skill.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
          {/* 状态标签 */}
          <Badge variant={config.variant} className={`${config.className} mr-6`}>
            {config.label}
          </Badge>
        </div>
      </CardHeader>
