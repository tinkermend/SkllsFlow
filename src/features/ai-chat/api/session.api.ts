...
export const sessionApi = {
  ...
  create: async (params?: CreateSessionParams): Promise<Session> => {
    ...
    try {
      ...
      console.log('[sessionApi] Database save successful:', backendResponse.data)
      ...
      try {
        ...
        console.error('[sessionApi] Failed to rollback OpenCode session:', rollbackError)
      }
  ...
  update: async (
    ...
    const openCodeClient = getOpenCodeClient()
    const response = await openCodeClient.patch<Session>(`/session/${id}`, params)
    return response.data
  },
}
