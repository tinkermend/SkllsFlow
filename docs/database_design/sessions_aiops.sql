...
-- 在 aiops schema 下创建 sessions 表和 enum
-- 基于 docs/database_design/sessions.sql 修改
...
-- 在 aiops schema 下创建 enum（注意：小写 session_status）
...
-- 验证创建成功
SELECT 'aiops.session_status enum:' as info,
...
SELECT 'aiops.sessions table:' as info,
       table_name
