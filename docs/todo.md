...
- [ ] `change:` 删除系统设置下的所有配置项,但不要删除配置项关联的页面, 删除后将系统设置改为系统管理,然后将用户管理挪移到系统管理中,点击系统管理展开后可以看到 用户管理
- [x] `feat:` 在系统管理下面新增 角色管理和菜单管理 页面, 相关的页面功能一方面你参考通用的web系统 角色管理和菜单管理的实现, 另外一方面你可以 阅读原来实现过的相关前后端rbac逻辑 @docs/rbac_plan2.md

- [ ] `change:` 基于你资深前端专家以及10年web系统开发的经验,你认为当前系统管理下面的 用户管理,菜单管理,角色管理,权限管理 页面布局与样式以及功能模块有哪些非常需要更改和优化的点,基于现有集成的shadcn前端框架
